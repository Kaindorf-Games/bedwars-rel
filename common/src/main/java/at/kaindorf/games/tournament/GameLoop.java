...
public class GameLoop extends BukkitRunnable {
    ...
    public void run() {
        ...
        CurrentState state = CurrentState.GROUP_STAGE;
        if (!groupStage.isFinished()) {
            ...
            state = CurrentState.KO_STAGE;
        }

        if(BedwarsRel.getInstance().isLeaderboardActive() && BedwarsRel.getInstance().getActiveLeaderboard() instanceof KoLeaderboard) {
            KoLeaderboard leaderboard = (KoLeaderboard)BedwarsRel.getInstance().getActiveLeaderboard();
            leaderboard.setStages(tournament.getGroups(), koStage, state);
        }
