...
public abstract class GameCycle {
    ...
    public void onPlayerDies(Player player, Player killer) {
        ...
        if (BedwarsRel.getInstance().statisticsEnabled()) {
            ...
            if (BedwarsRel.getInstance().isLeaderboardActive() && BedwarsRel.getInstance().getActiveLeaderboard() instanceof GroupLeaderboard) {
                GroupLeaderboard leaderboard = (GroupLeaderboard) BedwarsRel.getInstance().getActiveLeaderboard();
                leaderboard.addAttribute(String.valueOf(player.getUniqueId()), "Deaths");
            }
            ...
            if (BedwarsRel.getInstance().isLeaderboardActive() && BedwarsRel.getInstance().getActiveLeaderboard() instanceof GroupLeaderboard && killer != null) {
                GroupLeaderboard leaderboard = (GroupLeaderboard) BedwarsRel.getInstance().getActiveLeaderboard();
                if (teamIsDead) {
                    leaderboard.addAttribute(String.valueOf(killer.getUniqueId()), "Final Kills");
                } else {
                    leaderboard.addAttribute(String.valueOf(killer.getUniqueId()), "Kills");
                }
    ...
    private void runGameOver(Team winner) {
        ...
        // Leaderboard Stats
        if (BedwarsRel.getInstance().isLeaderboardActive() && BedwarsRel.getInstance().getActiveLeaderboard() instanceof GroupLeaderboard && winner != null) {
            List<String> uuids = winner.getPlayers().stream().map(p -> String.valueOf(p.getUniqueId())).collect(Collectors.toList());
            GroupLeaderboard leaderboard = (GroupLeaderboard) BedwarsRel.getInstance().getActiveLeaderboard();
            leaderboard.addAttribute(uuids, "Wins");
        }
